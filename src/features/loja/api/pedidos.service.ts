import type { Pedido, CriarPedidoDTO, AtualizarStatusPedidoDTO } from '../types/pedido';
...
export async function criarPedido(dados: CriarPedidoDTO): Promise<Pedido> {
  try {
    ...
    // Calcular totais
    const subtotal = dados.itens.reduce((sum, item) => sum + (item.price * item.quantity), 0);
...
export async function cancelarPedido(id: string, motivo?: string): Promise<Pedido | null> {
