...
} from "../api/pedidosCliente.service";
...
export function PagamentoPage() {
    ...
    async function carregarPedido() {
      ...
      try {
        const pedidoEncontrado =
          await buscarPedidoClientePorId(pedidoId);
        ...
        if (pedidoEncontrado) {
          ...
          setMostrarQRCode(
            pedidoEncontrado.formaPagamento === "pix",
          );

          setMostrarParcelas(
            pedidoEncontrado.formaPagamento === "cartao_credito",
          );
        }
      } catch (error) {
        setErro(
  ...
  async function handlePagar(e: React.FormEvent) {
    ...
    try {
      ...
      if (pedidoId) {
        ...
        await atualizarStatusPedidoCliente(pedidoId, {
          status: "confirmado",
  ...
  const totalComJuros =
    parcelas > 1
      ? Number(
          (valorParcelaAtual * parcelas).toFixed(2),
        )
      : valorTotal;
  ...
  return (
    <section className="pagina-loja pagamento-page">
      ...
      <p className="pagina-loja__introducao">
        Revise o pagamento do seu pedido e confirme para
        enviá-lo à cozinha.
      </p>
      ...
      >
          ...
          <div className="formulario-pagamento__campo parcelas-container">
            ...
            <div className="parcelas-resumo">
              <div className="parcelas-info">
                <span>
                  {parcelas === 1
                    ? `À vista: R$ ${valorParcelaAtual.toFixed(
                        2,
                      )}`
                    : `${parcelas}x de R$ ${valorParcelaAtual.toFixed(
                        2,
                      )}`}
                </span>
        ...
        >
          {loading ? (
            ...
            parcelas > 1 ? (
            `Pagar ${parcelas}x de R$ ${valorParcelaAtual.toFixed(
              2,
            )}`
          ) : metodo === "pix" ? (
            "Confirmar PIX"
