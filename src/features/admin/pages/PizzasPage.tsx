...
export function PizzasPage() {
  ...
  const [pizzaEditando, setPizzaEditando] = useState<Pizza | undefined>(
    undefined,
  );
  ...
  function handleSubmit(dados: PizzaFormData) {
    if (pizzaEditando) {
      editarPizza(pizzaEditando.id, dados);
  ...
  function handleEditar(pizza: Pizza) {
  ...
  return (
    <main>
      ...
      <PizzaForm
        key={pizzaEditando?.id ?? "nova-pizza"}
        pizza={pizzaEditando}
      ...
      {carregando && <p>Carregando pizzas...</p>}

      {erro && <p role="alert">{erro}</p>}

      {!carregando && !erro && pizzas.length === 0 && (
      ...
      {pizzas.map((pizza) => (
        <article key={pizza.id}>
          ...
          {pizza.imagem && (
            <img src={pizza.imagem} alt={`Pizza ${pizza.nome}`} width="200" />
          )}

          <div>
            <button type="button" onClick={() => handleEditar(pizza)}>
              Editar
            </button>

            <button type="button" onClick={() => handleExcluir(pizza.id)}>
              Excluir
            </button>
          </div>
